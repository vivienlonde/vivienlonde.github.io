...
L'état marqué est l'un des $$ 2^n $$ états $$ \vert x \rangle $$. Remarquez que l'état de superposition uniforme a une amplitude de probibilité sur l'état marqué égale à $$ \sqrt{\epsilon} = \frac{1}{\sqrt{2^n}} $$. C'est bien plus que la probabilité $$ \epsilon = \frac{1}{2^n} $$ de choisir l'état marqué lors d'un tirage uniforme. Cette observation est vraie avant même d'avoir utilisé une seule fois l'oracle quantique $$ U_f $$. C'est une conséquence "gratuite" du fait que les états quantiques sont décrits par des **amplitudes de probabilité** qui sont des **racines carrées de probabilité**.
